...
<?php
	...
	if( $_GET['ac'] == 'tvpsave' )
	{
		if( $hlp->saveTVDetailHtml( $_POST['pagehtml'] ) )
			$hlp->echo_ok( 'html changed','tv' );
		else
			$hlp->echo_err( 'html change failed','tv' );
	}
	
	$tv_detail_html = $hlp->getTVDetailHtml();
	
	echo('<div style="border-bottom:1px #ccc dashed;">');
	
	echo("<form method=post action='$me?b=td&ac=tvpsave' >
			<div name=htmlpar style='padding-left:30px;border-bottom;1px #ccc dashed;padding-top:10px;height:400px;'>
				...
				<div name='viewhtml' style='width:90%;height:90%;padding-top:7px;border:1px #ccc solid;overflow:auto;'>$tv_detail_html</div>
				<div name='edhtml' style='display:none;width:90%;height:90%;padding-top:7px;'>
					<textarea id='pagehtml' name=pagehtml rows='20' cols='80' style='width: 95%'>$tv_detail_html</textarea>
				</div>
			</div>
		<form>");
